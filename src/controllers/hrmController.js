...
exports.upsertSalaryComponent = async (req, res) => {
  try {
    ...
    const isHR = ['HRM System Administrator', 'HR Manager', 'HR Specialist (Generalist)', 'CRM core Administrator', 'HR Director / Executive HR User'].includes(req.user.role);
...
exports.updateNetSalaryOnly = async (req, res) => {
  try {
    ...
    const isAuth = req.user.isPersonalTeamLeader || 
                   ['HR Manager', 'HRM System Administrator', 'CRM core Administrator'].includes(req.user.role);
...
const DEFAULT_TEMPLATES = [
  { key: 'nationalId', label: 'National ID', labelAr: '????? ??????', isDefault: true },
  { key: 'socialInsurance', label: 'Social Insurance Certificate', labelAr: '???? ?????????', isDefault: true },
  { key: 'militaryStatus', label: 'Military Status', labelAr: '???? ???????', isDefault: true },
  { key: 'graduationCertificate', label: 'Graduation Certificate', labelAr: '????? ??????', isDefault: true },
  { key: 'criminalRecord', label: 'Criminal Record / Fish', labelAr: '??? ?????', isDefault: true },
];
...
exports.getLeaveRequests = async (req, res) => {
  try {
    let query = {};
    const isHR = ['HRM System Administrator', 'HR Manager', 'HR Specialist (Generalist)', 'CRM core Administrator'].includes(req.user.role);
    if (!isHR) {
...
exports.updateDetailedSchedule = async (req, res) => {
  try {
    ...
    const isHR = ['HRM System Administrator', 'HR Manager', 'CRM core Administrator'].includes(req.user.role);
    if (!isHR) {
...
exports.getTrainings = async (req, res) => {
  try {
    let query = {};
    const hrmPrivileged = ['HRM System Administrator', 'HR Manager', 'CRM core Administrator', 'Training and Development Specialist'];
...
exports.updateTrainingReport = async (req, res) => {
  try {
    ...
    const isHR = ['HRM System Administrator', 'HR Manager', 'Training and Development Specialist', 'CRM core Administrator'].includes(req.user.role);
...
exports.upsertAuxSchedule = async (req, res) => {
  try {
    ...
    const isHR = ['HRM System Administrator', 'HR Manager', 'CRM core Administrator',
