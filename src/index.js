    ...
    cron.schedule('0 6 * * *', async () => {
      try {
        ...
        if (expiringLots.length > 0) {
          const managers = await User.find({ role: { $in: ['Inventory Manager', 'Warehouse Manager', 'CRM core Administrator', 'Core 360 Administrator'] }, isActive: true });
    ...
    cron.schedule('0 */4 * * *', async () => {
      try {
        ...
        if (breachedItems.length > 0) {
          const managers = await User.find({ role: { $in: ['Inventory Manager', 'CRM core Administrator', 'Core 360 Administrator'] }, isActive: true });
          for (const mgr of managers) {
